  ...
  BEGIN
    EXECUTE format( '
                      ...
                      REVOKE ALL PRIVILEGES
                          ON system_versioning.%3$I -- history table
                        FROM PUBLIC
                      ;

                      ALTER TABLE IF EXISTS
                                  %1$I.%2$I -- main table
                          INHERIT system_versioning.%3$I -- history table
                      ;
                      ...
                      SELECT stdlib.make_history_dependent( ''%1$I.%2$I''::regclass -- main table
                                                          , ''system_versioning.%3$I''::regclass -- history table
                                                          )
                      ;

                      CREATE OR REPLACE FUNCTION %1$I."%2$s_history_by_range"(tstzrange)
                        ...
                        ;
                      $system_versioned$; COMMENT ON FUNCTION %1$I."%2$s_history_by_range"(tstzrange) IS
                      E''Allow easy searching of %1$I.%2$I history by timestamp range''
                      ;

                      CREATE OR REPLACE FUNCTION %1$I."%2$s_history_by_timestamp"(moment timestamptz)
                                         RETURNS SETOF system_versioning.%3$I LANGUAGE sql STRICT STABLE PARALLEL SAFE AS $system_versioned$
                        SELECT *
                          FROM %1$I."%2$s_history_by_range"( tstzrange( moment
                                                                      , moment
                                                                      , ''[]''
                                                                      )
                                                           )
                        ;
                      $system_versioned$; COMMENT ON FUNCTION %1$I."%2$s_history_by_timestamp"(timestamptz) IS
                      E''Point in time accessor for %1$I.%2$I''
                      ;

                      CREATE VIEW %1$I."%2$s_history" AS
                           SELECT *
                             FROM system_versioning.%3$I
                      ;
    ...
    SELECT INTO pkey_column (array_agg(a.attname))[1]
           FROM pg_attribute  a
...
CREATE FUNCTION stdlib.get_config(p_name text, p_default bool)
        RETURNS bool LANGUAGE sql STABLE PARALLEL SAFE AS $$
  SELECT coalesce( stdlib.get_config( p_name )::bool
                 , p_default
                 )
  ;
